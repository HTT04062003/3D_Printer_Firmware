#include "i2clcd.h"
#include <stdbool.h>
I2C_HandleTypeDef hi2c1; // change handler accordingly
...
{
    location &= 0x7; // Only 8 custom characters can be created (0-7)
    lcd_send_cmd(0x40 | (location << 3)); // Set CGRAM address
    for (int i = 0; i < 8; i++)
    {
        lcd_send_data(pattern[i]);
    }
		lcd_send_cmd(0x80 | lcdPos[line][colum]);
